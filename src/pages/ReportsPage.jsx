...
function ReportsPage() {
  // Generate heatmap data once per render
  ...
  return (
    <div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-title">🗓️ Alert Heatmap (24h)</div>
          <div style={{ marginBottom: 6, fontSize: 10, color: "var(--text3)" }}>
            Hourly event density — today
          </div>
        ...
        <div className="card">
          ...
          {SLA_METRICS.map(m => (
            <div key={m.label} style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, fontSize: 12 }}>
              ...
              <span style={{ fontFamily: "var(--font-head)", fontWeight: 700, color: m.good ? "#22c55e" : "#f97316" }}>
                {m.val}
              </span>
            </div>
          ))}
      ...
      <div className="card">
        ...
        <table className="alert-table">
          <thead>
            <tr>
              <th>Report</th><th>Date</th><th>Type</th><th>Size</th><th></th>
            </tr>
          </thead>
          <tbody>
            {REPORTS.map((r, i) => (
              <tr key={i}>
                ...
                <td>
                  <span style={{ fontSize: 10, background: "var(--bg3)", color: "var(--text2)", padding: "2px 6px", borderRadius: 4 }}>
                    {r.type}
                  </span>
                </td>
                <td style={{ color: "var(--text3)" }}>{r.size}</td>
                <td>
                  <button className="btn" style={{ fontSize: 10, padding: "2px 8px" }}>⬇ Export</button>
                </td>
              </tr>
            ))}
          </tbody>
